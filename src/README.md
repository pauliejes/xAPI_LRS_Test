...
* `persistence` controls various settings for the storage of temporary files, and to read adhoc files
    * `statementStore` path where temporary files will be stored for use during stage2 statement structure validation
    ...
    * `conflicts` path where temporary files are stored for stage2 conflict validation
